...
function load_image(frm){
   ...
   if ( day > mcday ) {
      ...
      if ( julian_day < first_julian_day || julian_day > latest_julian_day ) {
         ...
         for (k=1; k<=nmodel; k++) {
            data=Num2Chr(k);
            filename="http://www.emc.ncep.noaa.gov/mmb/hchuang/web/fig/"+yr+"/"+dateStr+"/"+exp+".fireemis."+area+"."+dateStr+".location.png";
            preload(filename);
         }
         for (k=1; k<=nmodel; k++) {
            data=Num2Chr(k);
            filename="http://www.emc.ncep.noaa.gov/mmb/hchuang/web/fig/"+yr+"/"+dateStr+"/"+emcexp+".fireemis."+area+"."+dateStr+".location.png";
            preload(filename);
         }
