...
function anim(){
   ...
   document.highvaod_image.src =  pics[i+4*nmodel].src;
   document.fire1_image.src    =  pics[i+5*nmodel].src;
...
function show2(i){
   ...
   document.highvaod_image.src =  pics[ith+4*nmodel].src;
   document.fire1_image.src    =  pics[ith+5*nmodel].src;
}
   ...
   if ( day > mcday ) {
      ...
      if ( julian_day < first_julian_day || julian_day > latest_julian_day ) {
         ...
         for (k=1; k<=72; k++) {
            ...
            chr_oyr = String(obs_yr).padStart(2, '0');
            data=itmp;
            filename="http://www.emc.ncep.noaa.gov/regional/restricted/aqm/web/fig/"+chr_oyr+"/"+newdate+"/"+obsfhd+"."+area+".g19.aodc."+newdate+"."+data+".aod.high.png";
            preload(filename);
         }
         for (k=1; k<=72; k++) {
            itmp=cycle/1+k;
