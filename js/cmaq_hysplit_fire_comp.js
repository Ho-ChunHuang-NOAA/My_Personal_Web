...
function get_current_prod(){
  ...
  function myWin(){
    newWin = open ("http://www.emc.ncep.noaa.gov//regional/restricted/aqm/web/html/cmaq_hysplit_fire_comp.html", "displayWindow", "width=950,height=800,menubar=yes,resizable=yes,scrollbars=yes,toolbar=yes,location=yes,status=yes");
  }
...
function load_image(frm){
   ...
   if ( day > mcday ) {
      ...
      if ( julian_day < first_julian_day || julian_day > latest_julian_day ) {
         ...
         for (k=0; k<=2; k++) {
            data=Num2Chr(k);
            filename="http://www.emc.ncep.noaa.gov//regional/restricted/aqm/web/fig/"+yr+"/"+dateStr+"/t"+cycle+"z/hmsfire."+area+"."+h_exp+"."+dateStr+".t"+cycle+"z."+prod+".day"+k+".k1.png";
            preload(filename);
         }
         ...
         for (k=0; k<=2; k++) {
            data=Num2Chr(k);
            filename="http://www.emc.ncep.noaa.gov//regional/restricted/aqm/web/fig/"+chr_oyr+"/"+dateStr+"/t"+cycle+"z/cmaqhmsfire."+area+"."+c_exp+"."+dateStr+".t"+cycle+"z."+prod+".day0.k1.png";
            preload(filename);
         }
...
function load_image_latest(){
   ...
   for (k=0; k<=2; k++) {
      data=Num2Chr(k);
      filename="http://www.emc.ncep.noaa.gov//regional/restricted/aqm/web/fig/"+chr_yr+"/"+dateStr+"/t"+cycle+"z/hmsfire."+area+"."+h_exp+"."+dateStr+".t"+cycle+"z."+prod+".day"+k+".k1.png";
      preload(filename);
   }
   newdate=calendar_day_plus1(dateStr);
   obs_yr=extract_year_from_calendarD(newdate);
